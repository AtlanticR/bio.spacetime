...
spacetime = function( p, DATA, OUT=NULL, overwrite=NULL, DS=NULL, method="inla" ) {
  ...
  if (is.null(overwrite)) {
	...
	if (method=="spatial.covariance" ) { 
		if (is.null(overwrite) || overwrite) {
			spacetime.db( p=p, DS="bigmemory.dependent", B=DATA )
      spacetime.db( p=p, DS="bigmemory.coordinates", B=DATA  )
      spacetime.db( p=p, DS="bigmemory.covariates", B=DATA )
      spacetime.db( p=p, DS="bigmemory.statistics" ) # init output data objects
		}
    ...
    sS = spacetime.db( p, DS="bigmemory.statistics.status" )
  ...
  if (  method =="inla.interpolations" ) {
      ...
      spacetime.db( p=p, DS="bigmemory.coordinates", B=DATA  )
      spacetime.db( p=p, DS="bigmemory.covariates", B=DATA )
      spacetime.db( p=p, DS="bigmemory.prediction.coordinates", B=OUT )
      spacetime.db( p=p, DS="bigmemory.prediction.covariates", B=OUT ).

      spacetime.db( p=p, DS="bigmemory.statistics" )
      spacetime.db( p=p, DS="bigmemory.predictions" )
  	}
...
#        bathymetry.figures( DS="predictions.error", p=p )
  ...
  if (  method =="gam.harmonic" ) {
  	...
  	if (is.null(overwrite) || overwrite) {
        ...
        spacetime.db( p=p, DS="bigmemory.statistics" )
        spacetime.db( p=p, DS="bigmemory.predictions_st" ) -- need to implement
        spacetime.db( p=p, DS="bigmemory.predictions_covar_st" ) -- need to implement
  	}
